...
\caption{Event-study estimates: TWFE, LP-DiD \citep{dube2023}, and stacked DiD
  \citep{cengiz2019}. Outcome: $\ln(\texttt{avg\_players}+1)$.
  LP-DiD uses not-yet-treated controls at each period $k$
...
\caption{Separated TWFE specification ($\hat{\beta}_k$ coefficients with repeat-event
  scalar $\hat{\delta}$). Outcome: $\ln(\texttt{avg\_players}+1)$.
  Event-time dummies capture the first-giveaway effect;
...
\caption{Event-study estimates by pre-treatment popularity. Outcome: $\ln(\texttt{avg\_players}+1)$.
  Split at the pre-treatment median (59.2 avg players/month): high-traffic ($N = 215$ games,
  left) and low-traffic ($N = 214$ games, right). High-traffic pre-trend Wald test:
  $\chi^2 = 9.35$, $p = 0.589$. Low-traffic: $\chi^2 = 19.31$, $p = 0.056$.}
...
\caption{Callaway--Sant'Anna cohort-aggregated ATT estimates. Outcome: $\ln(\texttt{avg\_players}+1)$.
  ...
  are aggregated across 27 cohorts weighted by cohort size. Omitted period: $k = -1$.}
\label{fig:csatt}
\end{figure}
