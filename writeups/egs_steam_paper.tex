...
Steam concurrent players rise 5--14\% in the giveaway month, then decline to
10--23\% below baseline by month twelve. A downward pre-period event-study path indicates that games
selected for giveaways are already declining on Steam prior to treatment.
The short-run spike is consistent with the giveaway functioning as an advertising event;
the medium-run decline is primarily explained by the continuation of this pre-existing
lifecycle decline in selected titles, with platform substitution a possible but
secondary contributor. The LP-DiD estimate of approximately 10--12\% below baseline
...
When most gamers hear about a new video game, the first instinct is to open Steam and search for it.
Not the Epic Games Store, GOG, or any other platform. 
Steam is simply where games live in the average gamer's mental model. 
...
across the eighty distinct treatment cohorts in the data traces out the full
...
generate essentially zero additional effect.
...
Second, and most importantly for interpreting the medium-run reversal, games selected
for EGS giveaways appear to already be on a downward Steam trajectory prior to
treatment: the pre-period event-study coefficients for $k \in [-12, -1]$ trace a
negative slope relative to not-yet-treated controls, indicating that treated titles
were already losing engagement before the giveaway and that Epic preferentially selects
titles already in lifecycle decline. Much of the observed post-giveaway decline likely
reflects this pre-existing trajectory continuing, not a causal consequence of the
giveaway. Third, platform substitution (players migrating from Steam to their free
Epic copy) may contribute a residual causal component, but the LP-DiD estimate
(approximately 10--12\% below baseline) should be treated as an upper bound on this
substitution effect, not a point estimate of it.
...
The present paper differs from Clarkson's in three respects. First, and most
substantially, Clarkson's analysis focuses on the short-run contemporaneous effect;
the present paper estimates the full twelve-month post-treatment trajectory using a
...
contemporaneous-only design. Third, the sample construction differs: Clarkson's
dissertation focuses on platform traffic broadly, whereas this paper uses monthly
average concurrent players from SteamCharts as a direct measure of engagement
and matches the full EGS giveaway catalogue (471 unique titles, 446 matched to
Steam) rather than a curated subset.
...
the Epic Games Store from December 2018 through 2025, comprising 541 giveaway events corresponding
...
these have been manuallycorrected so that every title's earliest entry is flag-free and only
...
across games, with a large cluster of low-traffic titles (fewer than 20 median players)
...
metadata, precluding a direct comparison of single-player versus multiplayer titles
within the current analysis, a distinction that matters because platform-switching
costs differ substantially across those game types.
...
is more plausible to the extent that Epic's curation team does not systematically
...
jointly indistinguishable from zero. We test this formally with a joint Wald test
of $H_0\colon \theta_k = 0$ for $k \in [-12,-2]$. The marginal rejection of this test
($p = 0.026$) is consistent with a genuine negative pre-trend rather than purely a
statistical artifact; boundary-bin clipping (observations with $k < -12$ binned into
$k = -12$) likely inflates the specific coefficient at $k = -11$ but does not account
for the overall pre-period slope. We supplement the full pre-trend test with a test
restricted to $k \in [-5, -2]$ and with a test on the high-traffic subsample, where
lifecycle decline is less pronounced. Results are in Section~\ref{sec:results}.
...
concentrated in few periods. Here, the 80 treatment cohorts
are distributed across 80 distinct calendar months spanning December 2018 through
late 2025, so at any calendar month $t$ the panel contains games at a wide range
...
identification; no additional omissions are required given the richness of the
cohort spread.
...
a non-random subsample that would complicate interpretation. The cost of clipping
is that the boundary-bin estimates at $k = -12$ and $k = 12$ pool observations
from multiple true event-times and should be read as averages over all periods
beyond the window, not single-period estimates; interior estimates at
$k \in [-11,\, 11]$ are unaffected. The inflation at $k = -11$ noted in the
pre-trend discussion is a specific artifact of observations genuinely at $k < -12$
being absorbed into the $k = -12$ bin and pushing adjacent estimates upward.
...
($k = 0$), $\hat{\theta}_0 = 0.101$ (SE = 0.022, $p < 0.001$), implying
...
$\chi^2 = 21.77$ ($df = 11$, $p = 0.026$), marginally rejecting zero pre-trends.
This rejection is consistent with a genuine negative pre-trend in treated games
relative to controls, reflecting the selection of titles already in lifecycle decline.
Boundary-bin clipping (observations with $k < -12$ are binned into $k = -12$) likely
inflates the coefficient at $k = -11$ (0.105, $p = 0.007$) specifically, but clipping
alone does not account for the overall downward slope of the pre-period path across
$k \in [-12, -1]$. The broader pre-period trajectory is better interpreted as
evidence that parallel trends is violated for games selected by Epic: treated titles
were already on a declining trajectory before the giveaway, and that decline continues
into the post-period.

For the high-traffic subsample, the pre-trend Wald test yields $\chi^2 = 9.35$
($df = 11$, $p = 0.589$), a clean failure to reject. This suggests that the
pre-trend violation is concentrated in lower-traffic titles, where lifecycle decline
is more pronounced and boundary noise is larger, and that identification is cleaner
for the high-traffic subsample.

Across estimators, the pre-period coefficients for $k \in [-12, -1]$ trace a
negative slope: treated games are gradually losing ground relative to not-yet-treated
controls in the months leading up to the giveaway. This downward pre-period trajectory
is consistent with Epic preferentially selecting titles already in lifecycle decline,
whether because fading titles are cheaper to license or because publishers of declining
games are more receptive to giveaway terms. Under this interpretation, a meaningful
portion of the post-giveaway reversal is not caused by the giveaway but is simply the
continuation of a pre-existing downward trajectory. This selection concern is the
primary reason the LP-DiD estimate of $-0.121$ at $k = 10$ should be read as an
\emph{upper bound} on the true causal substitution effect rather than a point estimate.
...
\textbf{Pre-period patterns.} Across all three estimators, the pre-period coefficients
for $k \in [-12, -1]$ trace a negative slope: treated games lose ground relative
to not-yet-treated controls in the months approaching the giveaway. This downward
pre-period trajectory is the most direct evidence of selection on lifecycle decline;
Epic's curation appears to favour titles already on a downward Steam trajectory. TWFE
pre-period coefficients for $k \in [-5, -2]$ are individually small and insignificant
(all $|t| < 0.6$), but the overall pre-period slope from $k = -12$ onward is
negative. LP-DiD, which compares treated games only to genuinely not-yet-treated
controls, makes this pattern more visible: the negative pre-period slope is not
absorbed by fixed effects that condition on already-treated comparators who share the
same selection filter. Stacked DiD pre-period coefficients are generally small but
show a borderline-significant positive spike at $k = -6$ (0.060, CI [0.038, 0.083])
and $k = -7$ (0.026, CI [0.003, 0.050]); these may reflect pre-announcement media
effects, though this interpretation warrants caution.
...
on the reversal, significant at the 5\% level under all three estimators from $k = 5$
onward, is consistent with a medium-run decline driven primarily by the continuation
of pre-existing lifecycle decline in selected titles, with platform substitution as a
possible secondary contributor. Because the pre-period estimates show that treated
games were already declining before the giveaway, the LP-DiD medium-run estimate of
$-0.121$ should be read as an upper bound on the true causal substitution effect;
the actual causal impact on Steam engagement may be substantially smaller.

\subsection{Separated Specification: First vs.\ Repeat Giveaways}

Figure~\ref{fig:separated} presents the separated specification (Eq.~\eqref{eq:separated}).
The repeat-giveaway coefficient is $\hat{\delta} = 0.0001$ (SE = 0.061, $p = 0.999$), indistinguishable from zero. A game's second free giveaway on Epic adds
...
explanation is selection bias: Epic's curation team preferentially selects titles
whose Steam player base is already declining, whether because declining games are
cheaper to license or because publishers of fading titles are more willing to
accept giveaway terms. Under this mechanism, much of the observed post-giveaway
decline is simply the continuation of a pre-existing downward trend, not a causal
consequence of the giveaway. The clearest empirical evidence for this is the negative
slope of the pre-period event-study path across $k \in [-12, -1]$: treated games
were already losing engagement relative to not-yet-treated controls in the months
before the giveaway, and that trajectory continues into the post-period. Crucially,
this selection mechanism cannot explain the contemporaneous spike at $k = 0$ (which is
robustly causal and passes the randomization placebo); but it likely accounts for the
dominant share of the estimated medium-run reversal.
...
This mechanism is real but likely smaller in magnitude than the selection effect.
...
All three mechanisms are consistent with the observation that LP-DiD (which
conditions on richer cohort-specific pre-treatment trends) shows a materially smaller
reversal than TWFE\@. The approximately 0.08 log-point gap between TWFE ($-0.199$
at $k = 10$) and LP-DiD ($-0.121$) likely reflects the pre-existing lifecycle decline
that LP-DiD partially absorbs but TWFE attributes entirely to the giveaway. The
LP-DiD estimate of $-0.121$ is therefore best understood as an \emph{upper bound} on
the true causal substitution effect (the portion attributable to the giveaway itself)
rather than a point estimate; the actual causal impact may be substantially smaller
than 12\%.
...
The developer calculus is straightforward once the components are separated
correctly. The pre-existing lifecycle decline is irrelevant: that Steam trajectory
was going to happen regardless of whether the developer signs the deal, so it
should not enter the decision. The substitution component is also not a net
developer cost: a player who migrates from Steam to Epic is still playing the
game, and Epic's 12\% revenue cut is more favorable to the developer than Steam's
30\%. The deal therefore reduces to two clearly positive terms: the licensing
fee (paid upfront) and the advertising spike, our estimated $+5$--$14\%$
Steam engagement boost at $k = 0$. \citet{clarkson2025} shows Steam reviews
spike alongside concurrent players during giveaways, feeding the recommendation
algorithm and generating downstream sales; the Blood West case \citep{frvr2026}
suggests the purchase-conversion effect can far exceed the engagement effect.
For developers, accepting the EGS giveaway deal is almost always the correct
decision.

\subsubsection*{Should Steam be concerned about EGS giveaways?}

No. The giveaway program, as our results characterize it, is a net positive
...
The medium-run decline in Steam concurrent players is also not a meaningful
threat to Steam as a platform. Most of the observed decline is pre-existing
lifecycle decline in titles that were already losing Steam players before the
giveaway; Epic is not causing Steam games to lose players, it is selecting games
that were already losing them. The residual causal substitution component, at
most 10--12\% per LP-DiD, reflects a small number of players shifting activity
to a platform where they now hold a free copy. For Steam, these are marginal
players on declining titles, not the engaged users who sustain Steam's social
ecosystem.

More broadly, the giveaway program appears to reinforce rather than threaten
Steam's incumbency. The evidence that the advertising effect occurs on Steam,
that Steam reviews spike alongside concurrent players \citep{clarkson2025},
and that even free distribution through a rival platform drives players to
...
Steam concurrent players, Steam reviews increase \citep{clarkson2025}, and
...
Our results do, however, raise a pointed question about the program's efficiency.
...
Whether that is offset by durable EGS user acquisition is unknown, but it is
the right question to ask, and probably the most important one for evaluating
...
comparison (for instance, a genre-and-vintage-matched sample of games that were
considered but not selected for EGS promotions) could in principle sharpen
identification and partially relax the reliance on cohort timing; we leave this to
future work. Three mitigations are employed within the current design: the
stacked DiD and LP-DiD estimators restrict controls to not-yet-treated games,
the Callaway--Sant'Anna estimator avoids already-treated comparisons entirely,
...
\textbf{Selection of games in lifecycle decline.} Epic may preferentially choose games
whose Steam player base is already on a downward trajectory, violating parallel trends
for the post-giveaway reversal. The LP-DiD pre-period drift at $k = -4$ and $k = -3$
($-0.046$ and $-0.045$, marginally significant) is consistent with this pattern.
While LP-DiD and stacked DiD partially control for this by conditioning on
not-yet-treated comparators, they do not fully eliminate selection on trajectories
that emerge within the 12-month pre-window. The preferred LP-DiD estimate of the
reversal ($-0.121$ at $k = 10$) should therefore be interpreted as an upper bound on
the true causal substitution effect.
...
This paper provides large-scale causal evidence on the full dynamic
...
The results are consistent with advertising, lifecycle-selection, and substitution
mechanisms operating at different horizons. The giveaway generates media awareness
that temporarily lifts Steam traffic. Platform substitution (players migrating to
their free Epic copy) is a plausible secondary mechanism, but the LP-DiD estimate of
approximately 10--12\% below baseline should be treated as an \emph{upper bound} on
the true causal substitution effect; the actual causal impact on Steam engagement
may be substantially smaller.

The medium-run decline below baseline is primarily explained by the pre-existing
lifecycle decline of selected titles. The pre-period event-study coefficients for
$k \in [-12, -1]$ trace a negative slope, indicating that treated games were already
on a downward trajectory before the giveaway. The approximately 0.08 log-point gap
between TWFE and LP-DiD estimates reflects the portion of the observed reversal that
LP-DiD attributes to this pre-existing selection rather than to the giveaway itself.

Taken together, the findings suggest that Epic's free-game strategy may function more
as inadvertent Steam advertising than as a durable driver of Epic platform engagement.
For developers, the deal is almost always net positive: the licensing fee and
advertising spike are both clearly positive, and the substitution of players from
Steam to Epic is not a developer cost since those players are still playing the game
on a platform with more favorable revenue terms. Whether the program benefits Epic
itself is the more open question: the measurable engagement effects accrue to Steam,
and whether Epic recaptures value through durable EGS user acquisition is not
testable with the current data.
...
Despite heterogeneity across cohorts, the common pattern of a sharp positive
spike at $k=0$ followed by gradual mean-reversion is visible throughout,
corroborating the aggregate estimate.
...
\caption{Per-cohort Callaway--Sant'Anna event-study paths. Each thin line
  ...
  (early $\to$ late: purple $\to$ yellow, \textit{plasma} palette).
