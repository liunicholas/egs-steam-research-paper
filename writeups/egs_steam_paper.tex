...
that temporarily lifts Steam traffic. The subsequent medium-run decline below baseline
is primarily explained by the pre-existing lifecycle decline of selected titles: the
pre-period event-study coefficients for $k \in [-12, -1]$ trace a negative slope,
indicating that treated games were already on a downward trajectory before the giveaway,
and the approximately 0.08 log-point gap between TWFE and LP-DiD estimates reflects the
portion of the observed reversal that LP-DiD attributes to this pre-existing selection
rather than to the giveaway itself. Platform substitution (players migrating to their
free Epic copy) is a plausible secondary mechanism, but the LP-DiD estimate of
approximately 10--12\% below baseline should be treated as an \emph{upper bound} on the
true causal substitution effect; the actual causal impact of the giveaway on Steam
engagement may be substantially smaller. Taken together, the findings suggest that
Epic's free-game strategy may function more as inadvertent Steam advertising than as
a durable driver of Epic platform engagement, and that future research should directly
test whether giveaway-selected titles are systematically in lifecycle decline relative
to the broader gaming catalogue. For developers and platform operators weighing EGS
giveaway deals, the estimates imply that the short-run advertising benefit is real but
transient, while the medium-run decline accrues gradually, a pattern that likely
favors the deal for completed single-player titles but cuts against it for live-service
games where an active Steam player base has ongoing value.
