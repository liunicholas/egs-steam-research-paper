...
estimates as upper bounds on any causal substitution effect.
...
across 446 games given away from December 2018 through 2025. The identifying variation
...
likely closer to zero, since all three estimators absorb pre-existing
lifecycle decline only imperfectly and therefore retain some downward bias.
...
appearances.\footnote{Repeats cluster in two patterns: \textit{distant repeats},
where a game is offered again more than twelve months after its first
appearance, and \textit{proximate repeats}, where the same game is offered
within a few months or even days of the first, most notably during Epic's
annual holiday giveaway batches.} \tabref{tab:epic_csv} summarizes the giveaway
structure; we retain all giveaway dates for each game to support the
...
\texttt{steamcharts.com/app/\{appid\}}. The overall
...
titling such as subtitles, punctuation, and edition suffixes; \tabref{tab:pipeline}
...
games (one is dropped by the TWFE estimator as a singleton fixed effect,
leaving an effective regression sample of 38{,}641 game-months).
...
\tabref{tab:sumstats} reports summary statistics: the raw player count is
...
far better behaved (mean 4.43, SD 2.36). \figref{fig:outcome} compares the
pre- and post-giveaway distributions of the outcome and its unconditional mean
by event-time, with a sharp uptick visible at $k = 0$; \figref{fig:desc}
...
fraction of each cohort flagged as repeat).
...
Because treatment timing is staggered across seven years, the standard TWFE estimator
...
event-study estimators reported alongside TWFE in \secref{sec:results}, plus
...
\appref{app:csatt}, with the per-cohort decomposition in
\appref{app:cohort_paths}.
...
\figref{fig:compare} presents the pooled TWFE event-study
coefficients $\hat{\theta}_k$ from Equation~\eqref{eq:pooled}, with 95\%
confidence intervals clustered at the game level. \tabref{tab:coefs} reports
selected coefficients; the full 24-coefficient table is in
\appref{app:coeftable}. In the giveaway month ($k = 0$), $\hat{\theta}_0 = 0.101$
...
$-0.267$ (23.4\% below baseline) by $k = 12$. With the exception of $k = -11$
($\hat{\theta}_{-11} = 0.105$, $p = 0.007$), the pre-period coefficients
($k \in [-12, -2]$) are individually indistinguishable from zero, but the
full set visually drifts downward toward $k = -1$.

\subsection{Estimator Comparison}
\label{ssec:compare}

\figref{fig:compare} overlays the TWFE, LP-DiD, and stacked-DiD event-study
paths and \tabref{tab:estimator_compare} reports key coefficients.
...
statistically significant negatives by $k = 5$. The pre-period coefficients
drift downward across the window; aside from isolated exceptions (TWFE at
$k = -11$, LP-DiD at $k = -4$), the individual coefficients are statistically
indistinguishable from zero, and the formal joint pre-trend test is reported
below.

The estimators diverge sharply on the medium-run magnitude, and the direction
of that divergence is itself diagnostic. At $k = 12$, TWFE reaches $-0.267$,
LP-DiD $-0.105$, and stacked DiD a statistically insignificant $+0.016$. The
contemporaneous estimates are tighter (TWFE and stacked both $+0.101$, LP-DiD
$+0.131$). LP-DiD's slightly larger $k = 0$ estimate reflects an artifact of
its control group: not-yet-treated games are EGS-selected titles approaching
their own giveaway, themselves in the steepest phase of pre-treatment
lifecycle decline, which mechanically inflates the DiD contrast. The pattern
across the medium-run window is systematic: the more aggressively an estimator
absorbs calendar-time shocks within cohort stacks (stacked DiD $>$ LP-DiD $>$
TWFE in that dimension), the smaller the medium-run negative becomes. This is
the signature of a contaminating common shock that the stacked design absorbs
but TWFE does not. The three-estimator spread therefore brackets the medium-run
causal substitution effect between roughly $0$ and $-27\%$, with the preferred
stacked-DiD estimate at the less-negative end and losing significance by
$k = 12$. We return to interpretation in \secref{sec:discussion}.
...
$p = 0.035$ ($F_{11, 213} = 1.89$, $p = 0.042$). The pre-trend violation is therefore concentrated in lower-traffic titles,
where lifecycle decline is more pronounced. This is the empirical signature
of selection on already-declining titles: a contamination we can bound but
cannot fully eliminate without a clean never-treated comparison.
...
\figref{fig:popularity} presents the event-study estimates. High-traffic
...
statistics in \secref{ssec:pretrend}. The selection-on-decline concern
...
evidence is suggestive: the pre-trend tests in \secref{ssec:pretrend}
...
estimators (TWFE most negative, stacked DiD closest to zero) is consistent with both
...
rests entirely on timing variation across cohorts.\footnote{See
\secref{sec:method} for why an external never-treated pool was not
constructed.} A well-constructed never-treated comparison could in principle
...
pre-trend evidence reported in \secref{ssec:pretrend} is consistent with
...
the popularity split in \secref{ssec:pop} serving as a partial proxy.

% ─────────────────────────── 7. CONCLUSION ─────────────────────────────────
\section{Conclusion}
\label{sec:conclusion}

Using 446 games from 2018--2025 and a staggered difference-in-differences
design, this paper traces the full trajectory of Steam player engagement
around Epic Games Store free giveaways. We find a two-phase pattern. Steam
players rise by approximately 10--14\% in the giveaway month, a spike robust
across TWFE, LP-DiD, and stacked DiD and confirmed by a randomization placebo.
The effect then reverses, but the medium-run magnitude is estimator-dependent:
the preferred stacked-DiD estimate flattens to a statistically insignificant
$+0.016$ by month twelve, and the pre-trend violation in low-traffic titles
points to selection on already-declining games. Read together, the medium-run
negatives are best interpreted as upper bounds on any causal substitution
effect, which therefore appears small. The broader takeaway is that even free
distribution on a rival platform funnels measurable engagement back to the
incumbent --- consistent with the view that Steam's competitive moat lies in
community infrastructure that price competition cannot easily erode.
...
\begin{table}[H]
\centering\small
\caption{Epic Games Store giveaway dataset breakdown (Dec 2018--2025)}
\label{tab:epic_csv}
\begin{tabular}{lc}
\toprule
Category & Count \\
...
\noindent\small\textit{Notes:} The longest gap between a game's first and second
giveaway is 57 months (\textit{Kingdom Come: Deliverance}); the shortest
proximate repeats span only a few days within the same December holiday batch.
\end{table}
...
\noindent\small\textit{Notes:} RapidFuzz fuzzy string matching auto-matched 413
of 471 unique names; the remaining 58 received manual review (50 resolved, 8
confirmed absent from Steam). Fifteen matched entries are DLC or in-game content
mapped to the base-game App ID.
\end{table}
...
\noindent\small\textit{Notes:} Game--month panel, 446 games, July 2012--February 2026.
  The repeat-giveaway window indicator equals 1 in months within 12 months after
  a game's second (or later) Epic giveaway.
\end{table}
...
Game and calendar-month fixed effects; SEs clustered by game
($N = 38{,}641$; 446 games). Omitted period: $k = -1$.
...
SEs clustered by game; reference period $k = -1$. See
\secref{sec:method} for estimator definitions.
\end{table}
...
\caption{Steam player engagement around Epic free giveaways.
  \textbf{Left:} Density histogram of $\ln(\avgp+1)$ for pre-giveaway months
  ($k < 0$, blue) and post-giveaway months ($k \geq 0$, orange), all 446 games.
...
\caption{Panel structure and descriptive statistics.
  ...
  excluded for lack of pre-period data).
  \textbf{(b)} Number of games with first Epic giveaway in each calendar year
  (all 446 games).
  \textbf{(c)} Fraction of games in each cohort year that received any second
  or later Epic giveaway during the panel, proximate or distant (63 games
  total; exceeds the 30-game proximate-repeat count in
  \tabref{tab:epic_csv}).}
\label{fig:desc}
\end{figure}
...
\caption{Event-study estimates: TWFE ($\hat{\theta}_k$, Eq.~\eqref{eq:pooled}),
  LP-DiD \citep{dube2023}, and stacked DiD \citep{cengiz2019}. Outcome:
  $\ln(\avgp+1)$. Shaded bands are 95\% CIs (SEs clustered by game). Omitted
  category: $k = -1$. $N = 38{,}641$ game-months; 446 games. See
  \secref{sec:method} for estimator definitions.}
\label{fig:compare}
\end{figure}
...
\caption{Event-study estimates by pre-treatment popularity. Outcome:
  $\ln(\avgp+1)$. Sample split at the pre-treatment median (59.2 avg
  players/month) into high-traffic ($N = 215$) and low-traffic ($N = 214$).
  Pre-trend joint Wald tests: high-traffic $\chi^2_{11} = 9.04$, $p = 0.618$;
  low-traffic $\chi^2_{11} = 20.82$, $p = 0.035$.}
...
\figref{fig:csatt} presents the cohort-aggregated ATT estimates. The CS
...
\figref{fig:cohort_paths} overlays the full event-study path
...
\figref{fig:csatt}. Individual cohort trajectories are highly dispersed and
...
event. The clean-sample specification in \appref{app:robustness} drops
...
as \figref{fig:separated} makes visually clear, confirming that
...
\figref{fig:robustness} presents the pooled TWFE event-study on the clean
...
to the full-sample pooled TWFE (see \figref{fig:compare}), confirming that
...
\caption{Clean-sample robustness check. Pooled TWFE re-estimated after dropping all 30
  ...
  Coefficients are virtually identical to the TWFE path in \figref{fig:compare}.}
...
estimate is $-0.258$, compared with $-0.267$. \figref{fig:noholiday}
...
the resulting permutation null. \figref{fig:placebo} shows the
...
\tabref{tab:fullcoefs} reports all 24 event-time coefficients from the pooled TWFE
