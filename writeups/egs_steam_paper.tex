...
stacked DiD is the preferred causal estimate at $k = 0$ because its cohort-balancing
removes control-composition bias; LP-DiD serves as an upper bound on the medium-run
substitution effect for reasons discussed in Section~\ref{sec:results}. We employ
three complementary robustness estimators.
...
Among the robust estimators, stacked DiD is the preferred causal estimate at
$k = 0$ and LP-DiD provides an upper bound on the medium-run effect; the
reasons for this ordering are explained in Section~\ref{ssec:compare}.

\subsection{TWFE Event Study and Estimator Comparison}
\label{ssec:compare}
...
Figure~\ref{fig:compare} presents the pooled TWFE event-study coefficients $\hat{\theta}_k$
alongside LP-DiD and stacked DiD estimates, with 95\% confidence intervals.
Table~\ref{tab:coefs} reports the key TWFE coefficient estimates. In the giveaway month
($k = 0$), $\hat{\theta}_0 = 0.101$ (SE = 0.022, $p < 0.001$), implying approximately
a 10.6\% increase in Steam concurrent players. The effect remains marginally positive
at $k = 1$ ($\hat{\theta}_1 = 0.045$, $p = 0.047$) but turns negative and highly
significant from $k = 2$ onward, reaching $-0.145$ (13.5\% below baseline) by $k = 5$
and $-0.267$ (23.4\% below baseline) by $k = 12$. The pre-period coefficients
($k \in [-12, -2]$) are close to zero and individually insignificant for $k \leq -5$;
some positive drift appears at $k = -11$ and $k = -9$, which we discuss below.
Table~\ref{tab:estimator_compare} summarises the key coefficients across all three estimators.
...
(SE = 0.008, CI [0.032, 0.063]). The ordering is driven by control-group composition.
A DiD estimate equals the treated group's outcome change minus the control group's
outcome change, so a declining control group mechanically pushes the estimate upward.
LP-DiD's not-yet-treated controls are EGS-selected games approaching their own
giveaway, and are therefore in the steepest phase of pre-treatment lifecycle decline;
subtracting these declining controls inflates the LP-DiD estimate most. TWFE's
already-treated controls have passed through the post-giveaway spike and partially
stabilized at a lower level, so they are less actively declining and provide less
upward pull, placing TWFE between LP-DiD and stacked DiD. Stacked DiD constructs
balanced cohort-specific panels and weights each game equally across the stacks it
appears in, which neutralizes the control-composition effect and yields the most
conservative estimate of the contemporaneous effect.
...
DiD $-0.164$ (CI [$-0.201$, $-0.127$]). The same control-composition mechanism that
explains the $k = 0$ ordering operates in reverse at large $k$. LP-DiD's eligible
controls at $k = 10$ are games still at least ten months from their own giveaway,
placing them deep in their pre-treatment lifecycle decline. Subtracting these actively
declining controls attenuates LP-DiD's negative estimate toward zero, making it the
least negative estimator. TWFE's already-treated controls at large $k$ have completed
their post-giveaway trajectory and are declining less steeply, so less is subtracted
and the estimate is more negative. Stacked DiD sits between the two because cohort
balancing partially but not fully neutralizes the control-composition effect. The
spread across estimators reflects this systematic pattern rather than uncertainty
about the existence of the reversal: all three confidence intervals are entirely
below zero by $k = 5$.
...
coefficient of $\hat{\delta} = 0.0001$ (SE = 0.061, $p = 0.999$), indistinguishable
from zero. The $\hat{\beta}_k$ coefficients are numerically identical to the pooled
$\hat{\theta}_k$ to three decimal places, confirming that repeat-event contamination
is negligible and the pooled specification is appropriate.
...
All three mechanisms are consistent with the observation that LP-DiD shows a
materially smaller reversal than TWFE. The explanation is control-group composition,
not that LP-DiD conditions on richer trend controls. At large $k$, LP-DiD's eligible
controls are games still far from their own giveaway, placing them in the deepest
phase of pre-treatment lifecycle decline. Subtracting these actively declining controls
attenuates the negative estimate toward zero. The approximately 0.08 log-point gap
between TWFE ($-0.199$ at $k = 10$) and LP-DiD ($-0.121$) therefore reflects how
much of the observed decline is absorbed by the declining control group rather than
being attributable to the giveaway. The LP-DiD estimate of $-0.121$ is best understood
as an \emph{upper bound} on the true causal substitution effect rather than a point
estimate; the actual causal impact may be substantially smaller than 12\%.
...
\caption{Event-study estimates: TWFE ($\hat{\theta}_k$, Eq.~\eqref{eq:pooled}),
  ...
  Stacked DiD constructs cohort-specific balanced panels with not-yet-treated controls,
  weighted by $1/\text{number of stacks per game}$.
