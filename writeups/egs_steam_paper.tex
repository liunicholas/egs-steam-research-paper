...
10--23\% below baseline by month twelve. Pre-period drift in treated games relative
to not-yet-treated controls suggests that games selected for giveaways may already
be on a downward trajectory. The short-run spike is consistent with the giveaway 
functioning as an advertising event; the medium-run decline likely reflects 
a combination of platform substitution and selection of games already in lifecycle decline.
...
live in the average gamer's mental model, regardless of where they are sold or
...
This pattern shows evidence for both the advertising hypothesis and
substitution theory at different time horizons. The
giveaway announcement attracts attention to the title on Steam, generating a temporary 
engagement spike; but the free copy distributed via Epic gradually substitutes Steam 
engagement. Additionally, there could be selection bias for games already losing popularity
with a slight bump in engagement due to the giveaway.
...
select games at particular inflection points in their Steam life-cycle. The LP-DiD
pre-period estimates provide some evidence that this assumption is partially violated:
at $k = -4$ and $k = -3$, treated games show marginally significant negative
coefficients ($-0.046$ and $-0.045$) relative to not-yet-treated controls, consistent
with Epic preferentially selecting titles already in lifecycle decline. This selection
concern motivates our preference for the LP-DiD estimate as the most conservative
characterization of the causal reversal.
...
$k = 5$ onward — substantially strengthens the substitution or dying lifecycle interpretation.
...
\textbf{Selection of games in lifecycle decline.} A third and arguably the most
important mechanism is selection bias: Epic's curation team may preferentially select
titles whose Steam player base is already declining, whether because declining games
are cheaper to license or because publishers of fading titles are more willing to
accept giveaway terms. Under this mechanism, part of the observed post-giveaway
decline is simply the continuation of a pre-existing downward trend, not a causal
consequence of the giveaway. The LP-DiD pre-period estimates provide the clearest
empirical evidence for this: at $k = -4$ and $k = -3$, treated games show marginally
significant negative coefficients ($-0.046$ and $-0.045$) relative to not-yet-treated
controls, consistent with a pre-existing decline in selected titles. Crucially, this
selection concern cannot explain the contemporaneous spike at $k = 0$ (which is
robustly causal and passes the randomization placebo), but it likely accounts for a
substantial share of the estimated medium-run reversal.

All three mechanisms are consistent with the observation that LP-DiD (which conditions
on richer cohort-specific pre-treatment trends) shows a materially smaller reversal
than TWFE\@. The approximately 0.08 log-point gap between TWFE ($-0.199$ at $k = 10$)
and LP-DiD ($-0.121$) likely reflects the pre-existing lifecycle decline that LP-DiD
absorbs but TWFE attributes to the giveaway. The LP-DiD estimate of $-0.121$ is
therefore the preferred, more conservative characterization of the true causal
reversal; the TWFE estimate should be treated as an upper bound.
...
The results are consistent with both advertising and substitution mechanisms operating
at different horizons, though the interpretation of the medium-run decline warrants
caution. The giveaway generates media awareness that temporarily lifts Steam traffic;
the subsequent decline below baseline likely reflects a mix of platform substitution
and pre-existing lifecycle decline in selected titles, with LP-DiD evidence of
negative pre-treatment drift suggesting the latter is a meaningful contributor.
The LP-DiD estimate of approximately 10--12\% below baseline is the preferred causal
lower bound on the substitution effect; the larger TWFE estimate of up to 23\% likely
conflates substitution with selection. Taken together, the findings suggest that
...
giveaway deals, the estimates imply that the short-run advertising benefit is real
but transient, while the medium-run decline accrues gradually, a pattern that likely
