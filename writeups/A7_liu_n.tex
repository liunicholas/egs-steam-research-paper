...
relevant feedback is quoted in shaded boxes, followed by a brief response 
...
That said, your suggested fix (stacked DiD) is exactly right, and I have
implemented it. The stacked DiD \citep{cengiz2019} constructs cohort-specific
balanced panels with \textit{only} not-yet-treated games as controls within
each cohort stack, then estimates TWFE with cohort fixed effects. This
achieves the identification benefit of a never-treated group (clean controls,
no negative-weighting bias) without requiring an external sample. I also
implement Callaway--Sant'Anna cohort ATTs \citep{callaway2021}, which makes
the same restriction. Results from both are reported in Section~3.

\textit{Collinearity and omitted periods.}
Because I have 80 treatment cohorts distributed across 80 distinct calendar
months (December 2018 through late 2025), event-time dummies are identified
by \textit{within-calendar heterogeneity in event-time}: at any calendar
month $t$, the panel contains games at a wide range of event-times (recently
treated, approaching treatment, and far from it). This rich staggered
structure means the calendar-month fixed effects and event-time dummies are
not collinear, and omitting only the single reference period $k = -1$ is
sufficient for identification. No additional omission (and no pooling
assumption for the omitted periods) is needed.
...
\textbf{SUTVA.} A giveaway of game $A$ does not affect Steam engagement for
a different game $B$. This could fail if giveaways shift attention across
competing titles in the same genre.

\textbf{Exogenous timing.} Conditional on game fixed effects, the month Epic
selects a game is uncorrelated with that game's potential player-count
trajectory on Steam.
...
yields main estimates nearly identical to the full sample. Going forward I
treat repeat giveaways as a handled nuisance rather than a primary
complication: the base specification includes a scalar repeat-event indicator
to absorb any level shift, and results are robust to excluding these games
entirely.
...
This contemporaneous spike is statistically significant and economically
meaningful: a game that had 100 concurrent players in the month before its
Epic giveaway is estimated to have approximately 111 players in the giveaway
...
\noindent\small\textit{Notes:} Game and calendar-month fixed effects;
...
\caption{Event-study estimates: TWFE, LP-DiD \citep{dube2023}, and stacked
  DiD \citep{cengiz2019}. LP-DiD uses only not-yet-treated controls at each
  ...
  \textit{Source:} Author's calculations using SteamCharts.com data and Epic
  Games Store giveaway log \citep{dongre2025}.}
\label{fig:compare}
\end{figure}
