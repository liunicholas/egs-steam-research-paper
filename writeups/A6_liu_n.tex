...
month of each game's \textit{first} Epic free giveaway. Of the 446 treated games, 421
have SteamCharts data covering the treatment month itself, and 336 have a full
$[-12,+12]$ symmetric observation window that supports the event-study regression.
...
repeat-event indicator is a binary flag equal to 1 in any month falling within the
12-month post-treatment window of a game's \textit{second or later} giveaway; only
2\% of game--month observations carry this flag, reflecting that most games were given
away exactly once.
...
unimodal with a long right tail---but the post-giveaway distribution exhibits a
discernible rightward shift: the mean of the post distribution exceeds that of the
pre distribution by approximately 0.13 log points (roughly 14\%). The large mass
near zero in both distributions reflects the 123 games with fewer than 10 median
...
all games observed at that relative period. This is a raw analog of the event-study
coefficient plot: it does \textit{not} partial out game or calendar-time fixed
effects, so the level shifts partly reflect cohort composition differences between
games at different stages of their Steam life cycle. Nonetheless, two features of
...
\caption{Steam player engagement (log scale) around Epic free giveaways.
  ...
  the dashed vertical line marks the giveaway month ($k = 0$). Both panels use 38,641
...
pre-treatment median player count across 446 games is roughly bimodal---a large cluster
of low-traffic titles (fewer than 20 median players) and a smaller cluster of more
popular ones---motivating the popularity-stratified robustness checks in the main analysis.

\textbf{Panel (b): Treatment cohort distribution.} Treatment cohorts are broadly
evenly distributed across 2019--2023. The 2018 cohort is small (program launched
December 2018) and the 2024--2025 cohorts are smaller due to observation-window
truncation at the February 2026 scrape cutoff. The multi-year spread provides the
timing variation needed for the staggered DiD identification.
