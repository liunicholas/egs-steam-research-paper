...
\textbf{Final panel.} The assembled dataset contains 38,579 game-month observations across 445 games, with columns: \texttt{appid}, \texttt{steam\_name}, \texttt{epic\_name}, \texttt{month} (first of month), \texttt{avg\_players}, \texttt{peak\_players}, \texttt{first\_giveaway\_date}, and \texttt{all\_giveaway\_dates} (a list of every giveaway date for that game). From \texttt{all\_giveaway\_dates} I construct \texttt{repeat\_event\_it}, a binary indicator equal to 1 in any month $t$ that falls within the post-treatment window $[0, 12]$ of game $i$'s \textit{second or later} giveaway. The treatment variable is the calendar month of each game's first Epic giveaway. Because all 445 games derive from the Epic giveaway log, all are technically treated, and all 445 have an identified treatment month after correcting name-string discrepancies between the scraper cache and the giveaway CSV (see Section~\ref{sec:problems}). The distribution of treatment cohorts is heavily concentrated in 2019--2021, reflecting the period when Epic's giveaway program was at its most active.
...
\midrule
No SteamCharts data (HTTP 500) & 14 \\
\textbf{Games in final panel} & \textbf{445} \\
\quad With identified treatment month & 445 \\
\bottomrule
\end{tabular}
\end{table}
...
\ln(y_{sit} + 1) = \alpha_{si} + \gamma_{st} +
  ...
  + \varepsilon_{sit}
\end{equation}
$\hat{\theta}_k$ estimates the average effect of \textit{any} Epic giveaway event, treating first and repeat appearances symmetrically. This answers the broadest question---does an Epic giveaway, whenever it occurs, shift Steam engagement?---and serves as the most statistically powerful estimate because it uses all 515 event-windows (445 first + 70 repeat giveaways, after collapsing micro-batch clusters).
...
\textbf{Randomization placebo.} As an additional falsification test, I will randomly permute treatment dates across the 445 treated games (preserving cohort sizes) and re-estimate the event-study 500 times. If the $|\hat{\beta}_0|$ from the actual data lies in the tail of the permutation distribution, this confirms the result is not an artifact of the panel's time-series structure.
...
\textbf{Name-string discrepancies between scraper cache and giveaway CSV (resolved).} Seven games had mismatched \texttt{epic\_name} values between the per-game SteamCharts cache files and the cleaned Epic CSV---four had a spurious \texttt{(DLC)} suffix appended in the cache, two had capitalisation differences (\textit{Ark} vs.\ \textit{ARK}, \textit{For the King} vs.\ \textit{For The King}), and one had a suffix variant (\textit{(pack)} vs.\ \textit{(DLC pack)}). These have been corrected directly in the cache files and the panel CSV rebuilt; all 445 games now carry a \texttt{first\_giveaway\_date}.
